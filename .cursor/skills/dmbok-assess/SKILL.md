...
#### Backlogエクスポートの場合
ツール出力で DMBOK 領域に分類されたチケットやドキュメントのうち、以下を優先的に読む:

- **データ関連の主要チケット**（領域ごとに代表的なもの数件） — 実際の課題内容・議論の深さ
- **Documents / Wiki の設計書・定義書** — テーブル定義、ER図、設計書の品質と網羅性
- **運用手順書・ポリシー文書** — ガバナンス・セキュリティの整備状況

**読むべきファイルの目安**: 少なくとも 10〜20 ファイルは直接中身を確認する。ファイルが多い場合は領域ごとに代表的なものをサンプリングする。
...
2. **根拠** — スコアの判定理由（**直接読んだファイルの具体的な内容**を引用すること）
